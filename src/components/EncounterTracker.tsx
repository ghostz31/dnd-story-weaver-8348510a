...
const EncounterTracker: React.FC = () => {
  ...
  // Charger les données de la rencontre à partir des paramètres d'URL
  useEffect(() => {
    ...
    if (encounterData) {
      try {
        ...
        toast({
          ...
          description: `${participants.length} participants ajoutés.`
        });
      } catch (error) {
  ...
  const possibleConditions = [
    'Assourdi', 'Aveuglé', 'Charmé', 'Empoisonné', 'Entravé', 
    'Étourdi', 'Inconscient', 'Paralysé', 'Pétrifié', 'Terrorisé'
  ];
  ...
  return (
    <div className="container mx-auto p-4">
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold flex items-center gap-1">
                          {participant.isPC ? '👤' : '👹'} {participant.name}
                        </span>
      ...
      {/* Dialogue d'édition des points de vie */}
      <Dialog open={hpEditorOpen} onOpenChange={setHpEditorOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setHpEditorOpen(false)}>
              Annuler
            </Button>
