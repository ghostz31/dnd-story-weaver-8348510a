  ...
  try {
    ...
    if (!ac || !hp || !speed) {
      ...
      if (redBlock) {
        ...
        if (!ac) {
          const acMatch = redHtml.match(/Classe d'armure<\/strong>\s*(\d+)/);
          if (acMatch) {
            ac = acMatch[1].trim();
          }
        }
        ...
        if (!hp) {
          const hpMatch = redHtml.match(/Points de vie<\/strong>\s*([^<]+)/);
          if (hpMatch) {
            hp = hpMatch[1].trim();
          }
        }
        ...
        if (!speed) {
          const speedMatch = redHtml.match(/Vitesse<\/strong>\s*([^<]+)/);
          if (speedMatch) {
            speed = speedMatch[1].trim();
          }
        }
